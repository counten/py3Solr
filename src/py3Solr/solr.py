...
from .util import safe_urlencode, force_unicode, force_bytes, get_logger, sanitize, unescape_html, clean_xml_string, \
    long, DATETIME_REGEX, NESTED_DOC_KEY
...
class Solr(object):
    ...
    def _send_request(self, method, path="", body=None, headers=None, files=None):
        ...
        try:
            ...
            logger.exception(error_message, url, err)  # NOQA: G200
            ...
            logger.exception(error_message, method, url, err)  # NOQA: G200
    ...
    def add(
        ...
        """
        start_time = time.time()
        logger.debug("Starting to build add request...")
        solrapi = "XML"
    ...
    def extract(self, file_obj, extractOnly=True, handler="update/extract", **kwargs):
        ...
        try:
            ...
            logger.exception("Failed to extract document metadata")
            raise

        try:
            data = json.loads(resp)
        except ValueError:
            logger.exception("Failed to load JSON response")
            raise
